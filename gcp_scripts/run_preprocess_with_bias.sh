#!/bin/bash
# Preprocess data WITH signature bias

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/load_gcp_config.sh"

echo "Preprocessing WITH signature bias..."
...
gcloud compute ssh $INSTANCE_NAME \
    ...
    --command="cd $REMOTE_DIR && \
        ...
        2>&1 | tee logs/preprocessing_with_bias_\$(date +%Y%m%d_%H%M%S).log" \
    --tunnel-through-iap

if [ $? -eq 0 ]; then
    echo "Preprocessing WITH bias completed!"
    exit 0
else
    echo "Preprocessing WITH bias failed"
    exit 1
fi
