...
#
# Original relay extraction script by Fatema Tuz Zohora
# Adapted for SigRelayST
...
if __name__ == "__main__":
    ...
    parser.add_argument( '--output_path', type=str, default='output/', help='Output file name prefix according to user\'s choice')
...
#    target_relay = 'APOE-SDC4 to APOE-SDC1' #'PSAP-LRP1 to APOE-LRP1' #'PSAP-LRP1 to PSAP-LRP1'
    ...
    data_list['X']=[]
    data_list['Y']=[] 
    for i in range (0, min(100, len(two_hop_pattern_distribution))): #len(two_hop_pattern_distribution)):
    ...
    chart.save(output_path + 'SigRelayST_' + args.data_name +'_relay_pattern_histograms.html')
    ...
    data_list_pd.to_csv(output_path + 'SigRelayST_' + args.data_name +'_relay_pattern_count.csv', index=False)
    with gzip.open(output_path + 'SigRelayST_' + args.data_name + '_relay_pattern_cell_info', 'wb') as fp:
    ...
    print('Relay pattern extraction done and saved at:\n'+
          output_path + 'SigRelayST_' + args.data_name +'_relay_pattern_histograms.html' + '\n' +
          output_path + 'SigRelayST_' + args.data_name +'_relay_pattern_count.csv' + '\n' +
          output_path + 'SigRelayST_' + args.data_name + '_relay_pattern_cell_info'
          )

    '''
    with gzip.open(output_path + 'SigRelayST_' + args.data_name + '_relay_pattern_cell_info', 'rb') as fp:
    ...
    if len(edge_list_2hop) > 0:
        ...
        nt.from_nx(g)
        nt.save_graph(output_path + 'SigRelayST_' +args.data_name +'_relay_mygraph.html')
        print('Edge graph plot generation done and saved at: '+output_path + 'SigRelayST_' +args.data_name +'_relay_mygraph.html')
        ########################################################################
        # convert it to dot file to be able to convert it to pdf or svg format for inserting into the paper
        write_dot(g, output_path + 'SigRelayST_' + args.data_name + "_relay_test_interactive.dot")
        print('dot file generation done and saved at:'+output_path + 'SigRelayST_' + args.data_name + "_relay_test_interactive.dot")
