...
if __name__ == "__main__":
    ...
    elif args.signature_database != '':
        print(f'Warning: Signature database not found: {args.signature_database}')
        print('Proceeding without signature bias.')
            ...
            if i in weightdict_i_to_j and j in weightdict_i_to_j[i]: #dist_X[i,j]>0: #distance_matrix[i][j] <= args.neighborhood_threshold: 
                ...
                if len(cells_ligand_vs_receptor[i][j])>0:
                    for k in range (0, len(cells_ligand_vs_receptor[i][j])):
                        ...
                        # Get signature bias if available
                        signature_bias = 0.0
                        if use_signature_bias:
                            lookup_key = (gene.upper(), gene_rec.upper())
                            if lookup_key in signature_bias_dict:
                                signature_bias = float(signature_bias_dict[lookup_key])
                        
                        row_col.append([i,j])
                        # Add signature bias as 4th dimension if using signatures, otherwise keep 3D
                        if use_signature_bias:
                            edge_weight.append([weightdict_i_to_j[i][j], ligand_receptor_coexpression_score, cells_ligand_vs_receptor[i][j][k][3], signature_bias])
                        else:
    ...
    if use_signature_bias and len(edge_weight) > 0:
        edge_dim = len(edge_weight[0])
        print('Edge attributes dimension: %d (includes signature bias)'%edge_dim)
        # Count edges with non-zero signature bias
        edges_with_bias = sum(1 for ew in edge_weight if len(ew) == 4 and ew[3] > 0)
        print('Edges with signature bias > 0: %d (%.1f%%)'%(edges_with_bias, 100.0*edges_with_bias/len(edge_weight) if len(edge_weight) > 0 else 0))
    ...
    ################## required for the SigRelayST interactive version ###################
