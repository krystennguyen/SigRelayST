...
def get_split_graph(training_data, node_id_sorted, total_subgraphs, expression_matrix_path=''): # use this if you don't want to save the split graph into disk due to space issue
    ...
    # Detect edge dimension from edge_weight
    edge_dim = 3  # default
    if len(edge_weight) > 0:
        edge_dim = len(edge_weight[0])
    
    return graph_bag, num_feature, edge_dim    
...
def get_graph(training_data): # use this if you already have saved the split graph
    ...
    # Detect edge dimension from first graph
    edge_dim = 3  # default
    if len(graph_bag) > 0 and hasattr(graph_bag[0], 'edge_attr') and graph_bag[0].edge_attr is not None:
        if graph_bag[0].edge_attr.dim() == 2:
            edge_dim = graph_bag[0].edge_attr.shape[1]
    ...
    return graph_bag, num_feature, edge_dim, edge_dim #graph_bags
...
class Encoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, heads, dropout, edge_dim=3):
        ...
        self.conv =  GATv2Conv(in_channels, hidden_channels, edge_dim=edge_dim, heads=heads, concat = False)#, dropout=dropout)
        self.conv_2 =  GATv2Conv(hidden_channels, hidden_channels, edge_dim=edge_dim, heads=heads, concat = False)#, dropout=0)
...
def train_SigRelayST(args, graph_bag, in_channels, edge_dim=3):
    """Add Statement of Purpose
    Args: [to be]
    ...
    if args.load:
        ...
        DGI_model.load_state_dict(torch.load(DGI_load_path, weights_only=False))
        DGI_optimizer.load_state_dict(torch.load(args.model_path+'DGI_optimizer_'+ args.load_model_name  +'.pth.tar', weights_only=False))
...
#        torch.save(DGI_model.state_dict(), DGI_filename)
    ...
    """
    DGI_model.load_state_dict(torch.load(DGI_filename, weights_only=False))
    print("debug loss")
    DGI_loss = DGI_model.loss(pos_z, neg_z, summary)
